...
class CO2Gauge(Gauge):
    def __init__(self, width: int, height: int):
        super().__init__(name='CO₂', width=width, height=height)
        # register to setpoint load state
        events.co2_loader.register(self.set_load_state)
    ...
    async def reset_loaders(self, _: ICB) -> None:
        ...
        self.load_set = False
        self._logger.debug("Reset Loaders")
    ...
    def get_state(self) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        if icb.cm < 2:  # first check if co2 mode is not in controller state
            disabled = True
            value = 'OFF'
        elif icb.cc == -100:  # Now check for error state
            disabled = True
            value = 'ERR'
        else:  # nominal reading
            disabled = False
            value = '{} %'.format(icb.cc)
        current_set = str(icb.cp)

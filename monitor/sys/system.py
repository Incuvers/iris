...
def wfi(host: str, port: int, timeout: int) -> None:
    """
    Wait for it. Test a connection to a server on a network.

    :param host: hostname of address
    :type host: str
    :param port: port of address
    :type port: int
    :param timeout: time to wait for connection to be open
    :type timeout: int
    :raises TimeoutError: if timeout has been reached and connection is not established
    """
    BACKOFF = 0.2
    timer = 0
    result = False
    _logger.info("Testing connection to %s:%s with timeout of %ss", host, port, timeout)
    while not result:
        if timer >= timeout:
            raise TimeoutError
        result = test_connection(host, port, timeout=10)
        _logger.debug("Connection to %s:%s failed", host, port)
        time.sleep(BACKOFF)
        timer += BACKOFF
...
def write_img(img: bytes, filename: str) -> None:
    ...
    :type filename: str
    """
    base_path = os.environ.get('COMMON', '/etc/iris')
...
def read_lab_id() -> Optional[int]:
    ...
    """
    try:
        with open(os.environ.get('MONITOR_CERTS', '/etc/iris/certs') + '/lab_id.txt', 'r') as fp:
...
def write_lab_id(lab_id: Optional[int]) -> None:
    ...
    """
    fp = os.environ.get('MONITOR_CERTS', '/etc/iris/certs') + '/lab_id.txt'
...
def clear_thumbnail() -> None:
    ...
    filename = os.environ.get('COMMON', '/etc/iris') + "/thumbnail.png"
    try:
        os.remove(filename)
    except FileNotFoundError:
        pass
